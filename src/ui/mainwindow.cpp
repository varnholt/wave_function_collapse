...
MainWindow::MainWindow(QWidget* parent) : QMainWindow(parent), ui(new Ui::MainWindow)
{
   ...
   connect(ui->_generate, &QPushButton::pressed, this, &MainWindow::generate);
   ...
   connect(ui->_action_save, &QAction::triggered, []() { Config::instance().save(); });
   ...
   connect(
      ...
      [this]()
      {
         const auto path = QFileDialog::getSaveFileName(this, tr("project path"), QString(), "*.json");
         if (path.isEmpty())
         {
            return;
         }
...
void MainWindow::showSelectedTile(int32_t index)
{
   ...
   const auto scaled = copy.scaled(ui->_tile_label->size());
   ui->_tile_label->setPixmap(QPixmap::fromImage(scaled));
}
