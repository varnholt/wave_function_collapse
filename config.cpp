...
{
   const auto& it = std::find_if(_tiles.begin(), _tiles.end(), [tile_index](const Tile& t) { return t._tileset_index == tile_index; });
...
{
   auto it = std::find_if(_tiles.begin(), _tiles.end(), [tile_index](const Tile& t) { return t._tileset_index == tile_index; });
...
void Config::save()
{
   ...
   {
      if (tile._compatible_tiles[0].empty() && tile._compatible_tiles[1].empty() && tile._compatible_tiles[2].empty() && tile._compatible_tiles[3].empty())
      {
         continue;
      }
